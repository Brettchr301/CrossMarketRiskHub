...
def fetch_vote_counts(state: str, cycle: int, race_type: str) -> pd.DataFrame:
    ...
    # NYT returns full national data, not per-state, and takes (race_type, cycle)
    def _try_nyt():
        nyt_df = fetch_nyt_results_archive(race_type, cycle)
        if nyt_df.empty:
            return pd.DataFrame()
        # Filter to requested state if possible
        state_cols = [c for c in nyt_df.columns if c.lower() in ("state", "state_po", "state_abbr")]
        if state_cols:
            filtered = nyt_df[nyt_df[state_cols[0]].str.upper() == state.upper()]
            if not filtered.empty:
                return _standardize_dataframe(filtered, state, cycle, race_type, "NYT")
        return pd.DataFrame()
    ...
    for source_name, fetch_func in sources:
        try:
            df = fetch_func()
            if not df.empty:
                logger.info("Successfully fetched %s %d %s from %s", state, cycle, race_type, source_name)
                return df
        except Exception as exc:
            logger.warning("Source %s failed for %s %d %s: %s", source_name, state, cycle, race_type, exc)

    logger.warning("No data found for %s %d %s from any source", state, cycle, race_type)
